...
lazy_static! {
    ...
    pub static ref TASK_MANAGER: TaskManager = {
        ...
        let mut tasks = [TaskControlBlock {
            ...
            task_syscall_times: [0; MAX_SYSCALL_NUM], // 初始化系统调用次数
            task_time: 0,
        }; MAX_APP_NUM];
...
impl TaskManager {
    ...
    fn increment_current_task_syscall(&self, syscall_index: usize) {
        let mut inner = self.inner.exclusive_access();
        let current = inner.current_task;
        inner.tasks[current].task_syscall_times[syscall_index] += 1;
    }
...
/// get current task increment_current_task_syscall
pub fn increment_current_task_syscall(syscall_index: usize) {
    TASK_MANAGER.increment_current_task_syscall(syscall_index);
}